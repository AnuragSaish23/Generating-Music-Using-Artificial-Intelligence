...
class CellularAutomatonDrumGenerator:
    ...
    def add_random_fill(self, fill_length=4):
        """
        Adds a random fill at the end of the current drum pattern.

        Parameters:
            fill_length (int): Number of steps at the end to turn into a fill.
        """
        for i in range(1, fill_length + 1):
            pos = -i  # index from the end
            # Pick 2 instruments to activate randomly
            choices = random.sample(
                [DrumInstruments.KICK, DrumInstruments.SNARE, DrumInstruments.HIHAT, DrumInstruments.CLAP], 2
            )
            for instr in DrumInstruments:
                self.state[instr.value][pos] = 1 if instr in choices else 0
...
class DrumPatternMusic21Converter:
    ...
    def _instrument_to_music21_part(self, drum_instrument, state, pattern_length):
        ...
        """
        part = stream.Part()
        if drum_instrument == DrumInstruments.CLAP:
            clap_instr = instrument.Instrument()
            clap_instr.instrumentName ="Clap"
            part.insert(0, clap_instr)
        elif drum_instrument == DrumInstruments.KICK:
            part.insert(0, instrument.BassDrum())
        elif drum_instrument == DrumInstruments.SNARE:
            part.insert(0, instrument.SnareDrum())
        elif drum_instrument == DrumInstruments.HIHAT:
...
def main():
    ...
    total_bars = 8
    steps_per_bar = 16
    drum_generator = CellularAutomatonDrumGenerator(pattern_length=steps_per_bar)
    ...
    for bar in range(total_bars):
        drum_generator.step()
        all_states.append(drum_generator.state.copy())

        #add randomdill after 4 bars
        if(bar + 1) % 1 == 0:
            drum_generator.add_random_fill(fill_length=4)


    full_pattern = np.concatenate(all_states, axis = 1)
    score = music_converter.to_music21_score(full_pattern)
